...
// 2. 스트링 변환 :
//   - map() : 해당 스트림의 요소들을 함수의 인수로 전달하여, 그 반환값으로 이루어진 새로운 스트림을  만듦
//   - flatMap() : 각 요소를 변환하여 중첩된 스트림들을 평면화하여 새로운 스트림을 생성합니다.
// 3, 제한
//   - limit() : 해당 스트림의 첫 번째 요소부터 전달된 개수만큼 개수만큼의 요소만으로 이루어진 새로운 스트림 생성
//   - skip() : 해당 스트림의 첫 번째 요소부터 전달된 개수만큼의 요소를 제외한 나머지 요소만으로 이루어진 새로운 스트림 생성.

// 4. 정렬.
//   - sorted() 메소드는 해당 스트림을 주어진 비교연산자(comparator)를 이용하여 정렬
...
//
//        Stream<String> stream4 =Stream.of ("I study hard", "You studt Java", "I am hungry");
...
//        System.out.println();
//        //첫 번째 요소로부터 전달된 개수만큼의 요소만으로 이루어진 새로운 스트림을 반환
...
//        System.out.println();
          ...
          System.out.println();
          stream2.sorted(Comparator.reverseOrder()). forEach(s->System.out.println(s + " "));
